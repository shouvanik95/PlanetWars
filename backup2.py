...
def gainList(planetlist,planetweights,Target,turns,pw):
	shipsSent=0
	i=0
	losses=0
	captured=False
	...
	for i in range(0,len(planetlist)):
		...
		simstate=planetlist[i].simulateState(-1*int(planetlist[i].NumShips()*planetweights[i]),0)
		if(simstate[turns][1]==2):
			losses+=1
		if(captured==True):
...
def bestStrategy(turns,Target,superplanets,pw):
	myplanetlist=list()
	...
	for j in range(0,2):
		for i in range(0,planetsize):
			...
			for x in range(0,30):
				...
				Gain=gainList(sortedplanetlist,tempweights,Target,turns,pw)
				if(Gain-100*l1norm(tempweights)-2*proximity(Target,superplanets)>maxx):
					maxx=Gain-100*l1norm(tempweights)-2*proximity(Target,superplanets)
...
gainTurns=40
def gainvector(superplanets,pw):
	mystrategy=list()
	...
	for getthisplanet in superplanets:
		if(getthisplanet.getState(gainTurns)[1]!=1):
			...
			strategy=bestStrategy(gainTurns,getthisplanet,superplanets,pw)
...
def DoTurn(pw):
		...
		gvv=gainvector(superplanets,pw)
		...
		for gv in gvv:
			if(len(gv[0])!=0):
				...
				for i in range(0,len(my_planets)):
					logging.debug(gv[0][0][i].PlanetID())
