		...
		if(getthisplanet.Owner()==1):
			for i in range(0,planetsize):
				...
				if(pww[i]>0.5):
					...
					mystrategy.append(((myplanetlist,pww,100/allproximity(getthisplanet,superplanets)),getthisplanet))
					logging.debug((((myplanetlist,pww,100/allproximity(getthisplanet,superplanets)),getthisplanet)))
	logging.debug("Ending I")
	logging.debug(mystrategy)
	sortstrategy = sorted(mystrategy, key=lambda tup: (-1*tup[0][2]-gain2ndturn(tup[1],superplanets,closest3)+allproximity(tup[1],superplanets)))
	return sortstrategy
				...
				if(len(gv[0])!=0 and gv[0][2]>0.0):
					...
					for i in range(0,len(gv[0][0])):
						...
						#~ if(xxx[gv[0][0][i].PlanetID()]>1.0):
							#~ quitthis=True
					#~ logging.debug(xxx)
					if(gv[0][2]>0.0 and quitthis==False):
